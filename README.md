...
Calculates π to arbitrary precision using the Chudnovsky algorithm. Implementations included in C, C++, Rust and Python (plain and gmpy2-backed). Default behaviour is to compute 100000 digits. 
C/C++/Rust versions are high-performance (~100M digits on an i7 in ~2 minutes); pure Python will be very slow for large targets. Use suffixes (K, M, G, T) or scientific notation (1e6, 1E6).
...
Build & run — examples
Note: all implementations accept a decimal count argument (default 100000), numeric suffixes (K/M/G/T) and scientific forms (1e6, 1E6). They also accept --digits and --calculate flags (examples below use a small set of commands — the same patterns apply to the other scripts/binaries).

C
- Build:
  gcc -O3 pi_chudnovsky.c -o pi_chudnovsky -lgmp -lmpfr
- Run (default 100000 digits):
  ./pi_chudnovsky
- Run specific counts:
  ./pi_chudnovsky 1K
  ./pi_chudnovsky --calculate 2M
  ./pi_chudnovsky 1e6
...
- Run:
  ./pi_chudnovsky_cpp
  ./pi_chudnovsky_cpp 10M
  ./pi_chudnovsky_cpp --digits 5G
